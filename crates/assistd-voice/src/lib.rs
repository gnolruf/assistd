...
//!
//! Milestone 4 provides a `WhisperCpp` implementation of [`VoiceInput`];
//! milestone 5 provides a `Piper` implementation of [`VoiceOutput`]. The
//! traits are defined here so the daemon can hold them as
//! `Arc<dyn VoiceInput>` / `Arc<dyn VoiceOutput>` from the moment the
//! feature lands, without touching the protocol or handler shape.
...
/// milestone-4 implementation lands.
